...
class TestVersionGetters:
    ...
    def test_get_worker_version_returns_hardcoded_version(self):
...
class TestFormatVersionBanner:
    ...
    def test_format_version_banner(self, mock_flash, mock_runpod):
        result = format_version_banner()
        assert result == (
            f"Starting Flash Worker {__version__} | runpod-flash 1.5.0 | runpod 0.9.0"
        )

    @patch("version.get_runpod_version", return_value="unknown")
    @patch("version.get_flash_version", return_value="unknown")
    def test_banner_handles_unknown_versions(self, mock_flash, mock_runpod):
        result = format_version_banner()
        assert result == (
            f"Starting Flash Worker {__version__} | runpod-flash unknown | runpod unknown"
        )
